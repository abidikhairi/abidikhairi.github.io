...
- `research.html` (`/research/`), `projects.html` (`/projects/`), `skills.html` (`/skills/`), `experience.html` (`/experience/`), `blog.html` (`/blog/`) — one page per section at the repo root. Each is just front matter (`layout: default`, `title`, `permalink`) plus a single `{% include %}` of the matching partial in `_includes/`. There is no Contact page/section — it was removed.
...
- `_layouts/post.html` — layout for individual blog posts (extends `default`); renders title/date/tags and the post body in a `prose` card.
- `_includes/` — one partial per section (`hero.html`, `about.html`, `research.html`, `projects.html`, `skills.html`, `experience.html`, `blog.html`, `nav.html`, `footer.html`), plus reusable card partials (`project-card.html`, `timeline-item.html`). The section partials (research/projects/skills/experience/blog) all use `pt-24` top padding since each is now the first thing on its own page, directly under the fixed nav — keep that if you add another such page.
- `_posts/` — blog posts, Jekyll's native collection. Add a post by dropping in a new `YYYY-MM-DD-title.md` file with `layout: post` front matter (`title`, optional `tags`); Jekyll picks it up automatically and it shows up on `/blog/` and in `feed.xml`. Post permalinks are `/blog/:title/` (set via top-level `permalink:` in `_config.yml`).
- `_data/` — **all editable content lives here as YAML**: `site.yml` (nav — now real page URLs, not anchors — hero, short about summary, research, social links, footer), `projects.yml`, `skills.yml`, `experience.yml`, `education.yml`. To update bio text, add a project, or change skills/experience/education, edit these files — not the HTML partials. Blog posts are the one exception to "content lives in YAML," since they're long-form Markdown — those live in `_posts/` instead.
- `_config.yml` — Jekyll site config (title, description, author, markdown engine, post permalink pattern, plugins).
- `Gemfile` — pulls in the `github-pages` gem so local builds match GitHub's build environment. `Gemfile.lock` is gitignored (platform-specific; GitHub Pages' native build doesn't read it). GitHub Pages' allowed plugin set includes `jekyll-seo-tag` and `jekyll-feed` (both enabled in `_config.yml`); the local Docker/Bundler builds below get them for free since they ship inside the `github-pages` gem.
...
- The site uses the Tailwind **Play CDN**, which JIT-compiles whatever classes it finds in the rendered DOM in the browser — not a build-time purge step. The custom `navy` palette is declared inline in `_layouts/default.html` via `tailwind.config`. The CDN script tag also requests the `typography` Play CDN plugin (`?plugins=typography`), used by `_layouts/post.html`'s `prose` classes to style Markdown blog-post bodies. If this repo ever moves off the CDN build to a compiled Tailwind pipeline, that config needs to move to a real `tailwind.config.js`, the typography plugin would need to become a real devDependency, and any dynamically-built class strings would need to become static/safelisted.
